...
export async function prepareTransactionData(utxoSelection, toAddress, targetTokenAddress, amount) {
  ...
  for (let i = 0; i < utxoSelection.selectedUTXOs.length; i++) {
    ...
    console.log(`   - Safe Address: ${utxo.safeAddress}`);
       ...
       seiTestnet, getContractNetworks, Safe, privateKeyToAccount
     );
 ...
 * @param {Object} seiTestnet - Sei testnet chain config
 ...
 */
export async function prepareUTXOTransaction(utxo, spendingPrivateKey, stealthAddress, toAddress, targetTokenAddress, amount, seiTestnet, getContractNetworks, Safe, privateKeyToAccount) {
